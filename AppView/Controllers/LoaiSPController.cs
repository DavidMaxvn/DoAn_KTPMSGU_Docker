...
namespace AppView.Controllers
{
    public class LoaiSPController : Controller
    {
        ...
        public async Task<IActionResult> Show(int ProductPage = 1)
        {
            try
            {
                string apiUrl = "http://webapi/api/LoaiSP/getAll";
        ...
        public async Task<IActionResult> TimKiemLoaiSPTheoTen(string? ten, int ProductPage = 1)
        {
            try
            {
                ...
                string apiUrl = $"http://webapi/api/LoaiSP/TimKiemLoaiSP?name={ten}";
                var response = await _httpClient.GetAsync(apiUrl);
                string apiData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Create()
        {
            try
            {
                var responseLoaiSP = _httpClient.GetAsync(_httpClient.BaseAddress + $"http://webapi/api/LoaiSP/getAll").Result;
        ...
        public async Task<IActionResult> Create(LoaiSPRequest lsp)
        {
            try
            {
                lsp.ID = Guid.NewGuid();
                lsp.TrangThai = 1;
                string apiURL = $"http://webapi/api/LoaiSP/save";
        ...
        public async Task<IActionResult> Details(Guid id)
        {
            ...
            string apiUrl = $"http://webapi/api/LoaiSP/getById/{id}";
            var response = await _httpClient.GetAsync(apiUrl);
            string apiData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> GetLoaiSpById(Guid id, int ProductPage = 1)
        {
            ...
            try
            {
                string apiUrl = $"http://webapi/api/LoaiSP?id={id}";
                var response = await _httpClient.GetAsync(apiUrl);
                string apiData = await response.Content.ReadAsStringAsync();
                var LoaiSPs = JsonConvert.DeserializeObject<List<LoaiSP>>(apiData);
                return View(new PhanTrangLoaiSP

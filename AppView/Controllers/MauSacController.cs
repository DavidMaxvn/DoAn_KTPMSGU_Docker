...
namespace AppView.Controllers
{
    public class MauSacController : Controller
    {
        ...
        public MauSacController(IConfiguration config)
        {
        ...
        public async Task<IActionResult> Show(int ProductPage = 1)
        {
            try
            {
                string apiUrl = $"{_apiBase}/MauSac/GetAllMauSac";
        ...
        public async Task<IActionResult> SearchTheoTen(string? Ten, int ProductPage = 1)
        {
            try
            {
                ...
                string apiUrl = $"{_apiBase}/MauSac/TimKiemMauSac?name={Ten}";
                var response = await _httpClient.GetAsync(apiUrl);
                string apiData = await response.Content.ReadAsStringAsync();
                var users = JsonConvert.DeserializeObject<List<MauSac>>(apiData);
                if (users.Count == 0)
        ...
        public async Task<IActionResult> Create(MauSac ms)
        {
            try
            {
                ...
                else
                {
                    ...
                    string apiUrl = $"{_apiBase}/MauSac/ThemMauSac?ten={ms.Ten}&ma={encodedMauSac}&trangthai={ms.TrangThai}";
        ...
        {
            string apiUrl = $"{_apiBase}/MauSac/GetMauSacById?id={id}";
            var response = await _httpClient.GetAsync(apiUrl);
            string apiData = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> Edit(Guid id, MauSac ms)
        {
            ...
            try
            {
                ...
                {
                    string encodedMauSac = Uri.EscapeDataString(ms.Ma);
                    string apiUrl = $"{_apiBase}/MauSac/{id}?ten={ms.Ten}&ma={encodedMauSac}&trangthai={ms.TrangThai}";
        ...
        {
            string apiUrl = $"{_apiBase}/MauSac/{id}";

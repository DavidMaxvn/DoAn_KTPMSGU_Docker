...
window.onload = function () {
  ...
  var $inputMin = $("#inputMinPrice");
  var $inputMax = $("#inputMaxPrice");
  $inputMin.on("change", function() {
    var minVal = parseInt($inputMin.val()) || 0;
    var maxVal = parseInt($inputMax.val()) || 0;
    if (maxVal && minVal > maxVal) $inputMin.val(maxVal);
    if (minVal < 0) $inputMin.val(0);
    if (minVal && maxVal && minVal <= maxVal) {
      filtersAjax(createFilters("price", minVal + "-" + maxVal));
    }
  });
  $inputMax.on("change", function() {
    var minVal = parseInt($inputMin.val()) || 0;
    var maxVal = parseInt($inputMax.val()) || 0;
    if (minVal && maxVal < minVal) $inputMax.val(minVal);
    if (maxVal < 0) $inputMax.val(0);
    if (minVal && maxVal && minVal <= maxVal) {
      filtersAjax(createFilters("price", minVal + "-" + maxVal));
    }
  });
